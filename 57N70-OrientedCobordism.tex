...
\usepackage{pmmeta}
\pmcanonicalname{OrientedCobordism}
\pmcreated{2013-03-22 13:56:05}
\pmmodified{2013-03-22 13:56:05}
\pmowner{mathcam}{2727}
\pmmodifier{mathcam}{2727}
\pmtitle{oriented cobordism}
\pmrecord{7}{34694}
\pmprivacy{1}
\pmauthor{mathcam}{2727}
\pmtype{Definition}
\pmcomment{trigger rebuild}
\pmclassification{msc}{57N70}
\pmclassification{msc}{57Q20}
\pmsynonym{cobordant}{OrientedCobordism}
\pmsynonym{bordism}{OrientedCobordism}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Two oriented $n$-manifolds $M$ and $M'$ are called {\em cobordant} if there is an oriented $n+1$ manifold with boundary $N$ such that $\partial N=M\coprod M'^{opp}$ where $M'^{opp}$ is $M'$ with orientation reversed.  The triple $(N,M,M')$ is called a {\em oriented cobordism}.  Cobordism is an equivalence relation, and a very coarse invariant of manifolds.  For example, all surfaces are cobordant to the empty set (and hence to each other).

There is a cobordism category, where the objects are manifolds, and the morphisms are cobordisms between them.  This category is important in topological \PMlinkescapetext{quantum field theory}.
%%%%%
%%%%%
\end{document}
